...
\begin{document}
  ...
  \end{itemize}
    ...
    \textcolor{hwColor}{
      \\
      \includegraphics[height=6cm, width=12cm]{delta.JPG} \\
      \\
      \\
      \\
      ...
      \\
      $
        1=\left(4 wa-1\right)^2 e^{4wa} \Rightarrow 
        \begin{cases}
          wa \approx -0.63 \\
          \\
          wa \approx \Rightarrow \dfrac{\sqrt{-2mK}a}{\hbar}=0 \Rightarrow K=0\\
          \\
          wa \approx 0.36 \Rightarrow \dfrac{\sqrt{-2mK}a}{\hbar} \approx 0.36 \Rightarrow K=-\dfrac{0.06 \hbar^2}{ma^2}
        \end{cases}
      $ \\
      \\
      \\
      We can ignore the first result since it is a negative value (\emph{the square root of a positive number must be a positive}). 
      Since there is one negative energy, there is one bound state for this value of $\alpha$. \\
      \\
      \\
      $
        \psi(x)=\begin{cases}
        ...
        =\begin{cases}
          \left(C_3+C_4e^{2wa}\right) exp \left(\dfrac{1.10x}{a}\right), ~~~~~ x<-a \\
          \\
          C_3 exp\left(\dfrac{0.36x}{a}\right)+C_4 exp\left(-\dfrac{0.36x}{a}\right), ~~~~~ -a<x<+a \\
          \\
          \left(C_3 e^{2wa}+C_4\right) exp \left(-\dfrac{0.36x}{a}\right), ~~~~~ x>a
        \end{cases} \\
        \\
        \\
        \\
        =\begin{cases}
          \left[ C_4e^{2wa}\left(\dfrac{\hbar^2 w}{m \alpha}-1\right)+C_4e^{2wa}\right] exp\left(\dfrac{0.36x}{a}\right), ~~~~~ x<-a \\
          \\
          C_4 e^{2wa}\left(\dfrac{\hbar^2 w}{m \alpha}-1\right) exp\left(\dfrac{0.36x}{a}\right)+C_4 exp\left(-\dfrac{0.36x}{a}\right), ~~~~~ -a<x<+a \\
          \\
          \left[C_4 e^{4wa}\left(\dfrac{\hbar^2 w}{m \alpha}-1\right)+C_4\right]exp\left(-\dfrac{0.36x}{a}\right), ~~~~~ x>a
        \end{cases} \\
        \\
        \\
        \\
        =\begin{cases}
          3.09 C_4 exp\left(\dfrac{0.36x}{a}\right), ~~~~~ x<-a \\
          \\
          C_4 \left[exp\left(\dfrac{0.36x}{a}\right)+exp\left(-\dfrac{0.36x}{a}\right)\right], ~~~~~ -a<x<+a \\
          \\
          3.09 C_4 exp\left(-\dfrac{0.36x}{a}\right), ~~~~~ x>a
        \end{cases}
      $
      ...
      $
        \begin{cases}
          \dfrac{\hbar^2}{2ma} \geq \alpha ~ 1 ~ bound ~ state \\
          \\
          \dfrac{\hbar^2}{2ma} < \alpha ~~ 2 ~ bound ~ states
        \end{cases}
      $ \\
      \\
      \\
      ...
      \\
      \\
      The bound state energies in the limiting cases $a \rightarrow \infty ~ and ~ \infty$. \\
      \\
      \\
      $
        \begin{cases}
      ...
      $
        \begin{cases}
          ...
          C_6=C_3+C_4 \\
          \\
          C_1 \left(\dfrac{2m \alpha}{\hbar^2} -w\right)=-C_3w+C_4 w \\
          \\
          C_6 \left(\dfrac{2m \alpha}{\hbar^2} -w\right)=C_3 w-C_4 w 
        \end{cases} \\
        \\
        \\
        \Longrightarrow C_1 \left(\dfrac{2m \alpha}{\hbar^2} -w\right)=0 \\
      $
      \\
      \\
      If $C_1 \neq 0$ then 
      $
        \dfrac{\sqrt{-2mw}}{\hbar}=\dfrac{2m\alpha}{\hbar^2}  \\
        \\
        \\
        \\
        \therefore ~~~ w=-\dfrac{2m \alpha^2}{\hbar^2}
      $
      \\
      \\
      \\
      ...
      $
        ...
        \\
        \begin{cases}
          \left(\dfrac{2m \alpha}{\hbar^2} -w\right) \left(C_3 e^{-wa}+C_4e^{wa}\right)+C_3 we^{-wa}-C_4we^{wa}=0 \\
          \\
          \left(\dfrac{2m \alpha}{\hbar^2} -w\right) \left(C_3 e^{wa}+C_4e^{-wa}\right)-C_3 we^{-wa}+C_4we^{wa}=0 
        \end{cases} \\
        \\
        \\
      $
      Taking the limit when $a \to \infty$ \\
      \\
      \\
      $
        \begin{cases}
          C_4 e^{wa}\left(\dfrac{2m \alpha}{\hbar^2}-w\right)=C_4 e^{wa} \\
          \\
          C_3 e^{wa}\left(\dfrac{2m \alpha}{\hbar^2}-w\right)=C_3 e^{wa}
        \end{cases} \Longrightarrow 
        K=-\dfrac{m\alpha^2}{2\hbar^2}
      $ 
      \\
      \\
      \\
      Hence, \textbf{The single-well potential} has a bound state energy $K=-\dfrac{m\alpha^2}{2\hbar^2}$.
      \\
      \\
      And for the double-well potential we just need to plugin $2\alpha$ instead of $\alpha$ when $a \to 0$. \\
      \\
    }
